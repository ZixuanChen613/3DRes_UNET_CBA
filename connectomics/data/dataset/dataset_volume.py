...
class VolumeDataset(torch.utils.data.Dataset):
    ...
    def __init__(self,
        ...
        self.volume_size = [np.array(x.shape) for x in self.volume]  # volume size, [array([ 28, 426, 427])]  .....  could be multi-volume input [array([ 121, 2986, 2986])]
        self.sample_volume_size = np.array(sample_volume_size).astype(int)  # [array([ 285, 2012, 3012])]
        ...
        self.sample_num = np.array([np.prod(x) for x in self.sample_size])          # 11 x 35 x 36 = 13860 
        ...
        if self.mode in ['train','valid']:
            ...
            if self.augmentor is not None:  # augmentation
                if np.array_equal(self.augmentor.sample_size, out_label.shape):
                    ...
                    data = {'image':out_volume, 'label':out_label}              # [18 505 505]
            ...
            # output list
            out_target = seg_to_targets(out_label, self.target_opt) # TODO: 我们的目标  01分割 4-2-1实力边界掩码  ['0', '4-2-1']
