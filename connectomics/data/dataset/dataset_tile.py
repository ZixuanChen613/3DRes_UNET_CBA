...
class TileDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, chunk_num, chunk_num_ind, chunk_iter, chunk_stride,
        ...
        self.sample_volume_size = sample_volume_size    # [18, 392, 392]
        self.sample_label_size = sample_label_size
        self.sample_stride = sample_stride              # [1, 1, 1]
        ...
        if len(chunk_num_ind) == 0:
            # self.chunk_num_ind： 0-31 的list  [8, 2, 2]
        ...
        self.json_volume = json.load(open(volume_json))  # TODO: load json file. and images
        self.json_label = json.load(open(label_json)) if (label_json is not None and label_json!='') else None      # gt
        self.json_size = [self.json_volume['depth'],self.json_volume['height'],self.json_volume['width']]           # [128, 640, 640]
    ...
    def updatechunk(self, do_load=True): # load chunk
        if len(self.chunk_id_done)==len(self.chunk_num_ind):    # range(0,32)
            self.chunk_id_done = []
        id_rest = list(set(self.chunk_num_ind)-set(self.chunk_id_done)) # id_rest:0-31
        if self.mode == 'train':
            id_sample = id_rest[int(np.floor(random.random()*len(id_rest)))] # 4 random choose one chunk from (0, 32)
        elif self.mode == 'test':
            id_sample = id_rest[0]
        self.chunk_id_done += [id_sample] # update 已经处理的chunk

        zid = float(id_sample//(self.chunk_num[1]*self.chunk_num[2]))  # 14 // 4 = 3
        yid = float((id_sample//self.chunk_num[2])%(self.chunk_num[1])) # 14 // 2 % 2 = 1
        xid = float(id_sample%self.chunk_num[2]) # 0
        #  self.chunk_step = 2, 50% overlap
        ...
        self.coord = np.array([z0,z1,y0,y1,x0,x1],int)          # 根据chunk_id map到的实际坐标 array([ 76, 128, 213, 640,   0, 426]) 
    ...
    def loadchunk(self):
        ...
        self.coord_m  array([  0, 128,   0, 640,   0, 640])
        coord_p array([ 85, 113,   0, 426, 213, 640])
        ...
        self.dataset = VolumeDataset(volume,label,  # [(121, 2986, 2987)]
                              self.sample_volume_size, # [34, 505, 505] [18, 392, 392]
                              self.sample_label_size, # [34, 505, 505]  [18, 392, 392]
