...
def seg_widen_border(seg, tsz_h=1):
    ...
    if len(sz)==3:
        ...
        patch[patch==0] = mm+1
        p1=patch.min(axis=1)
        seg =seg*((p0==p1).reshape(sz))         # modify on 1021
    return seg
...
def seg_to_targets(label, topts): # TODO: 产生label的地方  ['0', '4-2-1']
    # input: (D, H, W)
    # output: (C, D, H, W)
    out = [None]*len(topts)
    for tid,topt in enumerate(topts):
        ...
        elif topt[0] == '4': # instance boundary mask   '4-2-1'
...
def weight_binary_ratio(label, mask=None, alpha=1.0): # TODO : balance
