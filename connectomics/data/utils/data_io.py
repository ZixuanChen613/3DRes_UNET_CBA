...
def readim(filename, do_channel=False):
    ...
    if not os.path.exists(filename):
        raise RuntimeError("{} is not existing!!!".format(filename))
...
def readimg(filename):
    ...
    # 初始化 NumPy 数组，保持与之前 readimgs 的输出一致
    data = np.zeros((1, img.shape[0], img.shape[1]), dtype=np.uint8)
    ...
    for z in range(z0, z1):
        ...
        for row in range(r0, r1):
            for column in range(c0, c1):
                ...
                # print(path)
                patch = readim(path, do_channel=True)   # TODO: 可以读入内存优化。
