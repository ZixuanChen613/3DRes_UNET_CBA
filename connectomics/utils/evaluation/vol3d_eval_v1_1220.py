...
class VOL3Deval:
    ...
    def get_dtm_by_area(self, area_id):
        ...
        # False Negatives (FN): 未被任何检测结果正确匹配的 Ground Truth
        fns = np.zeros((self.T, len(self.gid)))  # 初始化 FN 矩阵
        for t in range(self.T):  # 遍历每个 IoU 阈值
            matched_gt_ids = self.gid[np.isin(self.gid, dtm[t, :])]
            unmatched_gt_ids = np.setdiff1d(self.gid[gtIg == 0], matched_gt_ids)
            fns[t, :] = np.isin(self.gid, unmatched_gt_ids).astype(int)

        npig = (gtIg==0).sum()
        return tps, fps, fns, npig
...
#             print('Please run evaluate() first')
        ...
        accuracy    = -np.ones((T, A))  # 新增 accuracy 结果矩阵            
        ...
        for a, a0 in enumerate(a_list):
            tps, fps, fns, npig = self.get_dtm_by_area(a)
            if npig == 0:
                continue

            tp_sum = np.cumsum(tps, axis=1).astype(dtype=np.float64)
            fp_sum = np.cumsum(fps, axis=1).astype(dtype=np.float64)
            fn_sum = np.cumsum(fns, axis=1).astype(dtype=np.float64)

            for t, (tp, fp, fn) in enumerate(zip(tp_sum, fp_sum, fn_sum)):
                total_samples = tp[-1] + fp[-1] + fn[-1]  # 总样本数（忽略 TN）
                if total_samples > 0:
                    accuracy[t, a] = tp[-1] / total_samples  # 准确度计算
...
#             'counts': [T, R, K, A, M],
    ...
    def summarize(self):
        ...
        '''
        def _summarizeAccuracy():
            accuracy = self.eval['accuracy']
            mean_accuracy = np.mean(accuracy[accuracy > -1])  # 计算有效的平均准确度
            print(' {:<18} = {:0.3f}'.format('Average Accuracy', mean_accuracy))
        ...
        def _summarizeDets():
            ...
            stats[3] = _summarize(1, iouThr=.3)#, maxDets=self.params.maxDets[2])
            stats[4] = _summarize(1, iouThr=.4)#, maxDets=self.params.maxDets[2])
            ...
            stats[10] = _summarize(1, iouThr=.75)#, maxDets=self.params.maxDets[2])
...
class Params:
    ...
    def setDetParams(self):
        ...
        ####
        thr_list = np.round(np.linspace(.1, .9, int(np.round((0.9 - .1) / .1) + 1), endpoint=True), decimals=2)
        # 2. 将 0.75 合并进去
        thr_list = np.concatenate([thr_list, [0.75]])  
        # 3. 为了防止重复，并保持有序，可以去重 + 排序
        thr_list = np.unique(thr_list)
        thr_list.sort()
        self.iouThrs = thr_list
        #####
