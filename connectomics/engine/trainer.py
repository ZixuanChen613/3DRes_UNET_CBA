...
from connectomics.utils.processing.postprocess_ssem import *
from connectomics.utils.evaluation.iteration_eval import cal_infer
from scripts.tools.plot_training_data import save_im_seg_bd_images

from scripts.tools.plot_val_data import save_seg_bd_ins_images
...
class Trainer(object):
    ...
    def count_model_size(self):
        ...
        size_in_bytes = total_params * 4  # 假设参数是 float32 类型
    ...
    def train(self):
        ...
        for iteration in range(self.total_iter_nums):       # 2500
            iter_total = self.start_iter + iteration
            start = time.perf_counter()

            # load data
            batch = next(self.dataloader)
            _, volume, target, weight = batch       # volume (2, 1, 16, 192, 192)
            time1 = time.perf_counter()

            # prediction
            volume = torch.from_numpy(volume).to(self.device, dtype=torch.float)            # torch.Size([2, 1, 16, 256, 256])
            pred = self.model(volume)

            # 在模型预测前，可视化第8层深度的切片 
            # save_im_seg_bd_images(volume, target, pred, iter_total, save_dir="train_visualizations")
    ...
    def test_valid(self, cfg, model_name): # TODO: testing stage.
        ...
        total_num_vols = len(self.dataloader) * self.cfg.INFERENCE.SAMPLES_PER_BATCH            # 27 x 12
        ...
        with torch.no_grad():
            for _, (pos, volume) in enumerate(self.dataloader):
                ...
                # forward pass
                output = test_augmentor(self.model, volume)             # (12, 2, 16, 256, 256)
        ...
        ## need to modify
        # bc_w_result = dis_watershed_test([semantic_mask, instance_boundary])

        # Save as .npy files
        np.save("semantic_mask_0614.npy", semantic_mask)
        np.save("instance_boundary_0614.npy", instance_boundary)
        print("save finish")
        ############### need to modify on 0406

        instance_id, _ = improved_instance_segmentation(semantic_mask, instance_boundary, 
                                                     semantic_threshold=0.9,
                                                     boundary_threshold=12,
                                                     mask_threshold=200,
                                                     seedpoint_threshold=500)

        updated_instance_id = solve_undersegmentation(instance_id)
        bc_w_result = updated_instance_id
        print("updated_instance_id")
        ############### need to modify on 0406
        ...
        # modify on 2024.10.30
        # save_seg_bd_ins_images(semantic_mask, instance_boundary, bc_w_result, output_dir="output_images_0.9_0.8")
    ...
    def test(self, cfg, model_name): # TODO: testing stage.
        ...
        if ("super" in self.cfg.MODEL.ARCHITECTURE):
            ...
            # result = [np.stack([np.zeros(x, dtype=np.float32) for _ in range(NUM_OUT)]) for x in self.dataloader._dataset.volume_size]
            # weight = [np.zeros(x, dtype=np.float32) for x in self.dataloader._dataset.volume_size]
            result = [np.stack([np.zeros(x, dtype=np.float16) for _ in range(NUM_OUT)]) for x in self.dataloader._dataset.volume_size]
            weight = [np.zeros(x, dtype=np.float16) for x in self.dataloader._dataset.volume_size]
        ...
        # Save as .npy files
        # np.save("semantic_mask_crop2_1202.npy", semantic_mask)
        # np.save("instance_boundary_crop2_1202.npy", instance_boundary)
    ...
    def  run_chunk(self, mode, cfg, args):
        ...
        if mode == 'train':
            num_chunk = self.total_iter_nums // self.cfg.DATASET.DATA_CHUNK_ITER    # 80 = 200k / 2.5k
            self.total_iter_nums = self.cfg.DATASET.DATA_CHUNK_ITER                 # 2500
            for chunk in range(num_chunk):                  # 80 = 200k / 2.5k
