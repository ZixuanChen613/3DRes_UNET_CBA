...
class CrossSliceAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        x_view = x.permute(0, 3, 4, 2, 1).view(-1, depth, channels)
...
class unetr3d_transformer_csa(nn.Module):
    ...
    def __init__(self, in_channel=1, out_channel=3, filters=[28, 36, 48, 64, 80], pad_mode='rep', norm_mode='bn', act_mode='elu', 
        ...
        self.cross_slice_attention = nn.ModuleList([
            CrossSliceAttention(in_channels=filters[x+1], num_heads=4, embed_dim=64) for x in range(self.depth)
        ])

        self.boundary_attention = nn.ModuleList([BoundaryAwareAttention(filters[x+1]) for x in range(self.depth)])


        # Cross Attention 交叉注意力的定义和通道调整卷积
        self.cross_attn = nn.ModuleList([CrossAttention(filters[x+1], filters[x]) for x in range(1, self.depth+1)])
    ...
    def forward(self, x):
        ...
        for i in range(self.depth):
            ...
            if i > 0:  # 跨层注意力
                down_u[i] = self.cross_attn[i-1](down_u[i], down_u[i-1])
