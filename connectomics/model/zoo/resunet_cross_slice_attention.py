...
class CrossSliceAttention(nn.Module):
    ...
    def forward(self, x):
        # x 的形状: (batch_size, channels, depth, height, width)
        batch_size, channels, depth, height, width = x.shape

        # 重塑为 (batch_size * height * width, depth, channels)
        ...
        # 恢复为原始形状 (batch_size, channels, depth, height, width)
        return output.view(batch_size, height, width, depth, channels).permute(0, 4, 3, 1, 2)


class resunet_cross_slice_attention(nn.Module):
    ...
    def __init__(self, in_channel=1, out_channel=3, filters=[28, 36, 48, 64, 80], pad_mode='rep', norm_mode='bn', act_mode='elu', 
        ...
        self.cross_slice_attention = nn.ModuleList([
            CrossSliceAttention(filters[x+1], num_heads=4, embed_dim=64) for x in range(self.depth)
        ])
    ...
    def forward(self, x):
        ...
        for i in range(self.depth):
            ...
            down_u[i] = self.cross_slice_attention[i](down_u[i])  # 应用跨切片注意力
